...
```
    今日进度：
        1.数字人管理平台：上传、语音库、数字人创建功能完善
```

day78:
```
    今日进度：
        1.数字人管理平台：应用部署、语音拷贝联调
        2.电子名片系统：中英文切换功能开发
```
...
```
    今日进度：
        1.数字人管理平台：2d数字人形象媒体文件删除功能、数字人展示功能开发
        2.电子名片系统：中英文切换功能开发
```
...
```
    今日进度：
        1.数字人管理平台：2d数字人形象媒体文件保存
        2.电子名片系统：名片样式分享功能、中英文切换功能开发
```
...
```
    今日进度：
        1.电子名片系统：名片样式-模板、分享功能开发
```

day74:
```
    今日进度：
        1.数字人管理平台：知识库与dify联调
        2.电子名片系统：名片样式-模板、背景、信息项展示功能开发
```

day73:
```
    今日进度：
        1.电子名片系统：文档管理功能列表开发、名片切换逻辑修改、样式调整
```
...
```
    今日进度：
        1.电子名片系统文件问题测试
        2.数字人管理平台：知识库功能接口完善、人物设定库功能接口完善
```
...
```
    今日进度：
        1.电子名片系统文件上传时端口映射问题解决
        2.数字人管理平台：知识库功能接口完善
        3.线性代数：
            - 复习矩阵转置、伴随、逆矩阵概念和运算
            - 矩阵的初等变换: 分为行变换和列变换
                1.交换矩阵的两行(列)  
                2.用数k(不为零)乘以某一行(列)的所有元素 
                3.把矩阵某一行(列)所有元素的L倍加到另一行(列)对应的元素上去
            - 标准形矩阵：元素只有两个数1和0组成，且矩阵的左上角是一个单位矩阵，其余元素全为零
            - 行阶梯形矩阵：1.如果矩阵存在零行，则零行都在非零行的下面；2.任一非零行从左到右第一个非零元素(称为首非零元)所在的列中元素(若还有)全为零.
            - 行简化阶梯形矩阵：特别地，若行阶梯形矩阵的首非零元都是1，且首非零元所在列上的其他元素都为零
```

day70:
```
    今日进度：
        1.电子名片系统部署上线（基本可用；待解决：文件上传时端口映射问题）
```
...
```
    今日进度：
        ...
        2.线性代数矩阵：
            - 方阵的幂：
                - 定义：设A为方阵，k为正整数，则A的k次幂为A^k = A * A * ... * A，规定A^0 = E
            - 矩阵的转置：
            - 矩阵的行列式：由n 阶方阵A 的元素所构成的行列式(各元素的位置不变),称为方阵 A 的行列式,记作|A| 或detA
                - 应该注意,方阵与行列式是两个不同的概念, n 阶方阵是n^2个数按一定方式排成的数表,而n阶行列式则是这些数(也就是数表A)按一定的运算法则所确定的一个数
                - 由 A 确定 |A| 的这个运算满足下述运算规律(设A,B为n 阶方阵,入为数）
                  1）：| A^T| = | A |(行列式性质1);
                  2）：| 入A | = 入 ^ n | A |
                  3）：| AB | = | A || B |
            - 矩阵的伴随矩阵：把矩阵 A 的行换成同序数的列得到一个新矩阵,叫做A的转置矩阵，记作A*
            - 矩阵的逆矩阵：逆矩阵满足：AB = BA = Е
                - 当A=0时,称为奇异矩阵,否则称非奇异矩阵,A是可逆矩阵的充分必要条件是| A | 不等于 0,即可逆矩阵就是非奇异矩阵.
                - 推论若AB = E(或 BA = E),则 B = A^-1
```
...
```
    今日进度：
        1.线性代数行列式：
            - 行列式定义：n阶行列式：n * n 个数排列成 n 行 n 列的数表
            - 全排列及其逆序数： 把 n 个不同的元素排列成一列，叫做这 n 个元素的全排列（也简称排列），n个不同元素的所有排列的种数通常用 Pn 表示， Pn = （n - 1）* (n - 2) * ... * 2 * 1 = n!
            - 逆序数：对于n个不同的元素，先规定各元素之间有一个标准次序(例如个不同的自然数，可规定由小到大为标准次序)，于是在这个元素的任一排列中，当某两个元素的先后次序与标准次序不同时，就说有1个逆序，一个排列中所有逆序的总数叫做这个排列的逆序数。逆序数为奇数的排列叫做奇排列，逆序数为偶数的排列叫做偶排列。
            - 主对角线以下(上)的元素都为0的行列式叫做上(下)三角形行列式，它的值与对角行列式一样。值为对角线元素的乘积，且为正。
            ...
            - 行列式的性质：
                ...
                - 性质5 若行列式的某一列(行)的元素都是两数之和,则可以拆分为两个行列式相加，其余行（列）值不变
                - 性质6 把行列式的某一列(行)的各元素乘以同一数然后加到另一列(行)对应的元素上去,行列式不变
            - 行列式按行(列)展开：一般说来,低阶行列式的计算比高阶行列式的计算要简便,于是,我们自然地考虑用低阶行列式来表示高阶行列式的问题,为此,先引进余子式和代数余子式的概念。
                - 在n 阶行列式中,把(i,j)元aij所在的第i行和第j列划去后,留下来的n-1阶行列式叫做(i,j)元的余子式,记作 Mij;记Aij = (-1)^(i+j) * Mij，Aij叫做代数余子式
                - 定理3 行列式等于它的任一行(列)的各元素与其对应的代数余子式乘积之和
            - 范德蒙德(Vandermonde)行列式：首行（列）为1，其余行（列）成比例，值为：Dn = （x2-x1)(x3-x1)...(xn-x1)...(xn-xn-1)
            - 推论行列式某一行(列)的元素与另一行(列)的对应元素的代数余子式乘积之和等于零
            - 拉普拉斯行列式：n行n列的余子式和代数余子式
            - 克拉默法则 如果线性方程组的系数行列式不等于零,则方程组有唯一解
            - 定理4 如果线性方程组的系数行列式 D≠0,则方程组一定有解,且解是唯一的
            - 定理5 如果齐次线性方程组的系数行列式 D≠0,则齐次线性方程组没有非零解。其逆定理为如果齐次线性方程组有非零解,则它的系数行列式必为零
        2.矩阵：
            - 定义：由m×n个数aij(i =1,2,…, m;j = 1,2, …,n)排成的m行n列的数表
            - 单位矩阵：行列相等的矩阵，也称为方阵，主对角线上的元素都是1，其余元素都是0，常用E表示
            - 矩阵的运算：矩阵必须是同型矩阵，即矩阵的行数和列数相等
              - 矩阵的加法与减法：符合交换律和结合律
                - A + B = B + A
                - A + (B + C) = (A + B) + C
              - 矩阵的乘法：两个矩阵的中间下标相等，如AB：A的列数等于B的行数，结果为A的行数B的列数的矩阵
                - 不完全符合交换律，只有AB=BA的时候符合交换律和结合律
